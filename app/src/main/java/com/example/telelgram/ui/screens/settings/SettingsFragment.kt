    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        when (item.itemId) {
            R.id.settings_menu_exit -> {
                ...
                AUTH.signOut()
//                (APP_ACTIVITY).replaceActivity(RegisterActivity())  заменено на рестарт
                restartActivity()
            }
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == CropImage.CROP_IMAGE_ACTIVITY_REQUEST_CODE && resultCode == RESULT_OK && data != null
            ...
            val path = REF_STORAGE_ROOT.child(
                ...
                .child(UID)  // путь по которой делаем upload
...
//                    path.downloadUrl.addOnCompleteListener { t ->  // получаеь download url/ адрес для скачтвания
//                        if (t.isSuccessful) {
//                            val photoUrl = t.result.toString()
//                            REF_DATABASE_ROOT.child(NODE_USERS).child(UID)  // метод записи в базу
